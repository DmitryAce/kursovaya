...
from .forms import NewItemForm, EditItemForm, FeedbackForm
...
def convert_layout(query):
    # Словарь для перевода символов между русской и английской раскладками
    ...
    # Возвращаем два варианта: оригинал и инвертированную раскладку
    return [query, query.translate(en_to_ru), query.translate(ru_to_en)]
...
def items(request):
    ...
    if query:
        ...
        stat, created = SearchStatistics.objects.get_or_create(query=query)
        stat.search_count = F('search_count') + 1
        stat.save()

        # Логика для ранжирования по раскладкам
        queries = convert_layout(query)
        ranked_items = []
        for q in queries:
            for item in items:
                score = (
                    fuzz.ratio(q, item.name)
                    + fuzz.partial_ratio(q, item.name)
                    + fuzz.token_sort_ratio(q, item.name)
                ) / 3
                ranked_items.append((item, score))
        ranked_items = sorted(set(ranked_items), key=lambda x: x[1], reverse=True)
...
def feedback(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            feedback.save()
            return JsonResponse({'message': 'Спасибо за отзыв!'})
    else:
        form = FeedbackForm()

    return render(request, 'item/feedback.html', {'form': form})
